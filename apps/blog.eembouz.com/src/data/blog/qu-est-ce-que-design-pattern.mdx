...
<blockquote>
  <p>
    ...
    naturellement dans l’architecture de mon store. En revanche, d’autres
    modèles comme **Builder**, **Strategy** ou **Decorator** sont plus
    **conceptuels** dans ce contexte. Ils seront donc illustrés de manière plus
    **générique en pseudo-code** pour faciliter leur compréhension. Ces exemples
    ne sont **pas destinés à être copiés tels quels** dans un projet Zustand ou
    React, mais plutôt à vous aider à **saisir l’idée générale** derrière chaque
    pattern. Vous verrez ensuite comment **adapter ces concepts** dans un projet
    réel si nécessaire.
  </p>
</blockquote>
